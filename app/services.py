...
class Services:
    ...
    def get_or_create_project(self, repo_name, repo_url):
        ...
        if existing_project:
            self.logger.info(
                f"Found existing project: {repo_name} (ID: {existing_project.get('id')})"
            )
            ...
            try:
                ...
                if project_id:
                    self.logger.info(
                        f"Successfully created project {repo_name} with ID: {project_id}"
                    )
                    return self.db.get_project_by_repo_name(repo_name)
                else:
                    self.logger.error(
                        f"Failed to create project {repo_name}: no ID returned"
                    )
                    return None
            except Exception as e:
                self.logger.error(
                    f"Exception while creating project {repo_name}: {str(e)}",
                    exc_info=True,
                )
                return None

    def update_project_deployment_info(
        self, repo_name, local_path=None, container_id=None, deployment_uuid=None
    ):
        ...
        try:
            ...
            if existing_project:
                ...
                self.logger.warning(
                    f"Project {repo_name} not found for deployment info update"
                )
        except Exception as e:
            self.logger.error(
                f"Failed to update deployment info for {repo_name}: {str(e)}",
                exc_info=True,
            )
    ...
    def log_deployment_status(
        ...
        self.logger.info(
            f"Logging deployment status: {status} for project ID: {project_id}"
        )
        ...
        try:
            ...
            if status == "success" and container_id:
                ...
                self.logger.info(
                    f"Updated project {project_id} with container ID: {container_id}"
                )

            # Additional logging for debugging
            if status == "started":
                self.logger.info(
                    f"{deployment_type.title()} deployment started for project {project_id}"
                )
                if commit_hash:
                    self.logger.info(f"  Commit hash: {commit_hash}")
            elif status == "success":
                self.logger.info(
                    f"{deployment_type.title()} deployment succeeded for project {project_id}"
                )
                if container_id:
                ...
                self.logger.error(
                    f"{deployment_type.title()} deployment failed for project {project_id}"
                )
                if error_message:
            ...
            self.logger.error(
                f"Failed to log deployment status: {str(e)}", exc_info=True
            )
    ...
    def get_project_urls(self, repo_name):
        ...
        self.logger.info(
            f"Generated {len(urls)} potential URLs for project {repo_name}"
        )
        return urls
    ...
    def get_deployment_metrics(self, repo_name=None):
        ...
        try:
            if repo_name:
                ...
                self.logger.info(
                    f"Retrieved {len(deployments)} deployment records for {repo_name}"
                )
                ...
                total_deployments = sum(
                    len(self.db.get_deployment_history(p["id"])) for p in projects
                )

                self.logger.info(
                    f"Retrieved metrics: {len(projects)} projects, {total_deployments} total deployments"
                )
            ...
            self.logger.error(
                f"Failed to get deployment metrics: {str(e)}", exc_info=True
            )
            return None
    ...
    def check_container_status(self, project):
        ...
        try:
            ...
            if not is_running:
                self.logger.debug(
                    f"Container {project['container_id']} for project {project['repo_name']} is not running"
                )
            ...
            self.logger.warning(
                f"Failed to check container status for {project['repo_name']}: {str(e)}"
            )
            return False
